"""OpenAPI spec validator adapter — implements PayloadValidator port."""
...
# ---------------------------------------------------------------------------
# Type coercion – Adyen accepts string integers/booleans in practice
# ---------------------------------------------------------------------------

def _coerce_types(node: Any) -> Any:
    """Recursively coerce string values to native JSON types.

    The Adyen API is lenient and accepts ``"10000"`` for integer fields
    and ``"true"``/``"false"`` for boolean fields.  This function
    normalises such values so the strict JSON Schema check does not
    reject them.
    """
    if isinstance(node, dict):
        return {k: _coerce_types(v) for k, v in node.items()}
    if isinstance(node, list):
        return [_coerce_types(item) for item in node]
...
class AdyenValidatorAdapter(PayloadValidator):
    ...
    def validate(self, payload: dict[str, Any]) -> list[dict[str, str]]:
        ...
        coerced_payload = _coerce_types(payload)
        validator = Draft4Validator(schema)
        errors: list[dict[str, str]] = []
        for error in sorted(validator.iter_errors(coerced_payload), key=lambda e: list(e.path)):
            errors.append({
